...
int step4(char* data[], char* ff_objid, char* ff_id, char* active_recip_objid )
{
   ...
   RC = PMSDK_MakeCall( pClass, "cl_assign_response_heat", pIList, subscribeList);
   if ( RC == 0 ) {
      is = 3360 ;
      RCC = PMSDK_MakeCall( pClass, "cl_respond_heat", pIList, subscribeList);
      if ( RCC == 0 ) {
         is = 3370 ;
      } else {
         is = -3370;
